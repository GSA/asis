...
describe MrssAlbumDetector do
  context 'when 4 or more other MRSS photos are sufficiently similar in their tags, title, and description fields' do
    before do
      5.times do |x|
        i = x + 1
        MrssPhoto.create(id: "photo #{i}", mrss_url: "http://some.mrss.url/feed.xml", tags: ['alpha', 'bravo', 'charlie', i.ordinalize],
    ...
    it 'should assign them all to the same album' do
      ...
      5.times do |x|
        i = x + 1
        expect(MrssPhoto.find("photo #{i}").album).to eq("http://some.mrss.url/feed.xml:2014-10-24:photo 1")
      end
    end
  end
end