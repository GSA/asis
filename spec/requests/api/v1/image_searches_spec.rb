...
describe API::V1::ImageSearches do
  describe "GET /api/v1/image_searches" do
    context 'when all params passed in' do
      ...
      before do
        expect(ImageSearch).to receive(:new).with("some query", { size: 11, from: 10, flickr_groups: ["fg1", "fg2"], flickr_users: ["fu1", "fu2"], instagram_profiles: ["ip1"], mrss_names: ["4", "9"] }).and_return(image_search)
      end

      it "performs the search with the appropriate params" do
        expect(image_search).to receive(:search) { search_results }
        get "/api/v1/image", query: "some query", size: 11, from: 10, flickr_groups: "fg1,fg2", flickr_users: "fu1,fu2", instagram_profiles: "ip1", mrss_names: "4,9"
